...
class POMDP:
    ...
    def get_state_str(self, state_num):
        '''
        Returns a string representing the state with the given num.
        This is the name given to it in the pomdp environment file.
        '''
        return self.pomdpenv.states[state_num]
    ...
    def get_observation_str(self, observation_num):
        ...
        This is the name given to it in the pomdp environment file.
        '''
        return self.pomdpenv.observations[observation_num]
    ...
    def sample_SOR(self, state_num, action_num):
        return self.pomdpenv.sample_SOR(state_num, action_num)
...
class POMDPEnvironment:
    def __init__(self, filename):
        ...
        while i < len(self.contents):
            #print '{}/{}'.format(i, len(self.contents))
            ...
            if line.startswith('discount'):
                ...
                # TODO: Load starting belief state from here.
                i += 1
            else:
    ...
    def sample_SOR(self, state_num, action_num):
        ...
        p_s_prime = [self.T[(action_num, state_num, s_num)] for
                     s_num in xrange(len(self.states))]
        s_prime = random.choice(range(len(self.states)), p=p_s_prime)
        p_o_prime = [self.Z[(action_num, s_prime, observation_num)] for
                     observation_num in xrange(len(self.observations))]
        o_prime = random.choice(range(len(self.observations)), p=p_o_prime)
        r = self.R[(action_num, state_num, s_prime, o_prime)]
